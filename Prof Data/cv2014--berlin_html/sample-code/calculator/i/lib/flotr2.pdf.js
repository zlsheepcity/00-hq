...
Flotr = {
  ...
  drawText: function(ctx, text, x, y, style) {
    ...
    // lipa antibold
    ctx.font = (style.weight > 1 ? "bold " : "") + (style.size*1.3) + clcFlotrSettings[clcFlotrView].font;
    //ctx.font = (style.weight > 1 ? " " : "") + (style.size*1.3) + clcFlotrSettings[clcFlotrView].font;
...
Flotr.defaultOptions = {
  ...
  fontSize: clcFlotrSettings[clcFlotrView].fontSize,           // => canvas' text font size
...
Text.prototype = {
  ...
  measureText : function (text, style) {
    ...
    context.save();
    // lipa antibold
    context.font = (style.weight > 1 ? "bold " : "") + (style.size*1.3) + clcFlotrSettings[clcFlotrView].font;
    //context.font = (style.weight > 1 ? " " : "") + (style.size*1.3) + clcFlotrSettings[clcFlotrView].font;
...
Graph.prototype = {
  ...
  calculateSpacing: function(){
    ...
    this.plotHeight = this.canvasHeight - p.bottom - p.top; //    lipa: fix years cutting
...
Flotr.addType('markers', {
  ...
  _plot: function(x, y, label, options) {
	...
	//BEGIN section added by intea
	else if (options.position.indexOf('a') != -1) {
			if (label>=0) {
				top -= dim.height + options.verticalMargin //like top
			} else { 
				top += options.verticalMargin; //like bottom
				//console.log(label); 
			}
	//END section added by intea
    ...
    if (!options.HtmlText && this.textEnabled) {
      ...
      if (options.title)
        html.push(
          ...
          //this.plotOffset.left, 'px;font-size:1em;font-weight:normal;text-align:center;width:',
      ...
      //html.push('<div class="flotr-axis-title" style="font-weight:normal;">');
